...
internal static class Program
{
  private static async Task Main()
  {
    ...
    var features = tzShapeReader.ReadShapeFile();

    Parallel.ForEach(
        Enumerable.Range(5, 2),
        precision =>
        {
          var outProject = precision == 5 ? "GeoTimeZone" : $"GeoTimeZone{precision}";

          ConsoleOutput.WriteMessage($"  - Building geohash data with precision {precision}...");
          var outputPath = Path.Combine(GetSolutionDir(), "src", outProject);

          var builder = new TimeZoneDataBuilder(features);
          builder.CreateGeohashData(precision, outputPath);
        }
    );
